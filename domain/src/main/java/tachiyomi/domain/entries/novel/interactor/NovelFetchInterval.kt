...
/**
 * Text based counterpart of the manga `MangaFetchInterval`. Reuses the exact same interval
 * heuristics so library update behaviour stays consistent across media types.
 */
class NovelFetchInterval(
    ...
    suspend fun toNovelUpdate(
        ...
        val interval = novel.fetchInterval.takeIf { it < 0 } ?: calculateInterval(
            chapters = getChaptersByNovelId.await(novel.id),
            zone = dateTime.zone,
        )
