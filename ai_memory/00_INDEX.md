...
- [task_20260805_1432_novel_phase6_fixes.md](task_20260805_1432_novel_phase6_fixes.md) — Fase 6 Novel: fix `NoClassDefFoundError NovelChapterNumberParser` (rule proguard novelsource di consumer-proguard.pro + app/proguard-rules.pro, hardening try-catch di `SyncNovelChaptersWithSource` & `EpubParser`); build+38 test hijau.
- [task_20260805_2222_novel_phase7_reader_settings.md](task_20260805_2222_novel_phase7_reader_settings.md) — Fase 7 Novel: revamp Detail UI ala MangaInfoScreen (EntryToolbar collapsing, NovelInfoBox, expandable desc, chapter list + filter unread/display), Reader Settings bottom sheet real-time (text size/font family/theme/line spacing/padding), konfirmasi extension NOVEL sudah ada; 42 test hijau.

## Status Proyek
- Build & seluruh test hijau: `testDebugUnitTest` (42 PASSED) BUILD SUCCESSFUL + `assembleDebug` BUILD SUCCESSFUL + `spotlessApply` bersih.
...
- Fase 6: `NoClassDefFoundError` chapter parser ditutup — kelas `NovelChapterNumberParser` terverifikasi ada & ter-package di APK debug/release, rule proguard novelsource ditambah, dan sync chapter resilient terhadap kegagalan parser.
- Fase 7: Detail Novel di-revamp ala MangaInfoScreen (collapsing toolbar, info box backdrop, deskripsi expandable + genre chips, daftar chapter + dialog filter unread & display mode judul/nomor); Novel Reader punya settings sheet real-time (text size, line spacing, padding, theme Light/Dark/Sepia, font family) dari `NovelReaderPreferences`; support extension NOVEL dikonfirmasi sudah ada di Fase 2-3 (`HttpNovelSource`/`ParsedNovelHttpSource` + `NovelExtensionManager`).
