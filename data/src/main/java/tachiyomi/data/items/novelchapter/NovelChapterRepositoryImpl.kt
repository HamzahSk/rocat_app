...
class NovelChapterRepositoryImpl(
    ...
    override suspend fun getChapterByNovelId(novelId: Long, applyScanlatorFilter: Boolean): List<NovelChapter> {
        return handler.awaitList {
            novel_chaptersQueries.getChaptersByNovelId(novelId, ::mapChapter)
        }
    }
    ...
    override fun getScanlatorsByNovelIdAsFlow(novelId: Long): Flow<List<String>> {
        return handler.subscribeToList { novel_chaptersQueries.getChaptersByNovelId(novelId, ::mapChapter) }
            .map { chapters -> chapters.map { it.scanlator.orEmpty() } }
    }

    override suspend fun getBookmarkedChaptersByNovelId(novelId: Long): List<NovelChapter> {
        return handler.awaitList {
            novel_chaptersQueries.getBookmarkedChaptersByNovelId(
                novelId,
                ::mapChapter,
            )
        }
    ...
    override suspend fun getChapterById(id: Long): NovelChapter? {
        return handler.awaitOneOrNull { novel_chaptersQueries.getNovelChapterById(id, ::mapChapter) }
    }
    ...
    override suspend fun getChapterByUrlAndNovelId(url: String, novelId: Long): NovelChapter? {
        return handler.awaitOneOrNull {
            novel_chaptersQueries.getNovelChapterByUrlAndNovelId(
                url,
                novelId,
                ::mapChapter,
            )
