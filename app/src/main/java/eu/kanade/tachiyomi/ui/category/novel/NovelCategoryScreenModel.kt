    ...
    fun createCategory(name: String) {
        screenModelScope.launch {
            when (createCategoryWithName.await(name)) {
                is CreateNovelCategoryWithName.Result.InternalError -> _events.send(
    ...
    fun hideCategory(category: Category) {
        screenModelScope.launch {
            when (hideCategory.await(category)) {
                is HideNovelCategory.Result.InternalError -> _events.send(
                    NovelCategoryEvent.InternalError,
                )
                else -> {}
            }
    ...
    fun renameCategory(category: Category, name: String) {
        screenModelScope.launch {
            when (renameCategory.await(category, name)) {
                is RenameNovelCategory.Result.InternalError -> _events.send(
                    NovelCategoryEvent.InternalError,
                )
                else -> {}
            }
