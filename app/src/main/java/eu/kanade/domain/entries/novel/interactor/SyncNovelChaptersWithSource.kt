...
/**
 * Text based counterpart of `SyncChaptersWithSource`. Synchronizes the chapters stored in the
 * database with the ones returned by a [NovelSource]. Novels don't use a download manager, so
 * no rename/has-download handling is needed.
 */
class SyncNovelChaptersWithSource(
    ...
    private val updateNovelChapter: UpdateNovelChapter,
    private val getChaptersByNovelId: GetChaptersByNovelId,
) {

    /**
     * Synchronizes db chapters with source ones.
     *
    ...
    suspend fun await(
        ...
        val dbChapters = getChaptersByNovelId.await(novel.id)
        ...
        for (sourceChapter in sourceChapters) {
            ...
            if (source is HttpNovelSource) {
                val sChapter = chapter.toSNovelChapter()
                source.prepareNewNovelChapter(sChapter, novel.toSNovel())
                chapter = chapter.copyFromSNovelChapter(sChapter)
            }
        ...
        if (newChapters.isEmpty() && removedChapters.isEmpty() && updatedChapters.isEmpty()) {
            if (manualFetch || novel.fetchInterval == 0 || novel.nextUpdate < fetchWindow.first) {
                updateNovel.awaitUpdateFetchInterval(
                    novel,
                    now,
                    fetchWindow,
                )
        ...
        var toAdd = updatedToAdd
        if (toAdd.isNotEmpty()) {
            toAdd = chapterRepository.addAllChapters(toAdd)
        }

        if (updatedChapters.isNotEmpty()) {
            val chapterUpdates = updatedChapters.map { it.toNovelChapterUpdate() }
            updateNovelChapter.awaitAll(chapterUpdates)
        }
        ...
        updateNovel.awaitUpdateLastUpdate(novel.id)

        return toAdd.filterNot {
