...
class NovelSourceRepositoryImpl(
    ...
    override fun getOnlineNovelSources(): Flow<List<DomainSource>> {
        return sourceManager.catalogueSources.map { sources ->
            sources
                .filterIsInstance<HttpNovelSource>()
    ...
    override fun getNovelSourcesWithFavoriteCount(): Flow<List<Pair<DomainSource, Long>>> {
        return combine(
            handler.subscribeToList { novelsQueries.getNovelSourceIdWithFavoriteCount() },
    ...
    override fun getNovelSourcesWithNonLibraryNovels(): Flow<List<NovelSourceWithCount>> {
        val sourceIdWithNonLibraryNovels =
            handler.subscribeToList { novelsQueries.getSourceIdsWithNonLibraryNovels() }
        return sourceIdWithNonLibraryNovels.map { sourceId ->
